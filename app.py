...
def getDB():
    ...
    db_url = os.getenv('DATABASE_URL', 'postgresql://jcudrwehso:P$XhJFtzc4P6o61I@group7-flask-postgresql-server.postgres.database.azure.com:5432/group7-flask-postgresql-database')
...
def check_csrf_token(func):
    @wraps(func)
    def decorated_function(*args, **kwargs):
        csrf_token = request.headers.get('X-CSRFToken')  # Lấy CSRF token từ headers
        if not csrf_token or csrf_token != get_csrf_token():  # So sánh token gửi đi và token trên server
            return jsonify({"error": "CSRF token missing or invalid"}), 401  # Lỗi nếu không hợp lệ
...
def view_blog(blog_title):
    ...
    with getDB() as (cursor, conn):
        try:
            ...
            cursor.execute(
                "SELECT id, title, content, likes, authorname, \"userID\", publish FROM \"blogPosts\" WHERE title = %s AND publish = TRUE",
                (decode_title,)
            )
            blog_post = cursor.fetchone()

            if blog_post:
                blog_id, title, content, likes, authorname, userID, publish_status = blog_post

                # Lấy bình luận liên quan đến bài viết này
                cursor.execute(
                    "SELECT username, comment FROM \"commentsBlog\" WHERE \"blogPostID\" = %s",
                    (blog_id,)
                )
